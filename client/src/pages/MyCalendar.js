...
export default function MyCalendar({userID="6630bee58763d8e65feee48f"}) {
  ...
  const [customID, setCustomID] = useState(userID);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const url = `http://localhost:4000/api/rsvp/${customID}`;
        ...
        <Row>
          <Container className="calendar-container">
            ...
            <div className="calendar-grid">
              {[...Array(Math.ceil(generateDaysArray(selectedDate).length / 7))].map((_, rowIndex) => (
                <Row key={rowIndex}>
                {generateDaysArray(selectedDate).slice(rowIndex * 7, (rowIndex + 1) * 7).map((dayObj, index) => (
                  <Col key={index} className="mb-3">
                    {console.log("Dates:", isInEventDates)}
                    {dayObj !== null ? (
                      dayObj.isInEventDates ? (
                      <Card
                        className={`calendar-card-event`}
                        onClick={() => handleDateClick(new Date(selectedDate.getFullYear(), selectedDate.getMonth(), dayObj.day))}
                      >
                        <Card.Body>
                          <div>
                            <div>{dayObj.day}</div>
